 ...
 * Mapper.java
 */
class MapperItfBuilder extends PubCodeBuilder implements CodeFileBulider
{
    @Override
    public void build(final GroundParamer gp) throws IOException {
        final File folder = new File(gp.getMapperPath());
        if (!folder.exists()) {
            folder.mkdirs();
        }
        final File mapperFile = new File(gp.getMapperPath(), String.valueOf(gp.getMapperName()) + ".java");
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(mapperFile), "utf-8"));
        bw.write("package " + gp.getMapperPakage() + ";");
        bw.newLine();
        bw.newLine();
        ...
        bw.newLine();
        bw = this.buildClassComment(bw, String.valueOf(gp.getMapperName()) + "\u6570\u636e\u5e93\u64cd\u4f5c\u63a5\u53e3\u7c7b");
        bw.newLine();
        bw.newLine();
        bw.write("public interface " + gp.getMapperName() + "{");
        bw.newLine();
        bw.newLine();
        ...
        bw.flush();
        bw.close();
        gp.setPreStepName(String.valueOf(gp.getMapperPakage()) + "." + gp.getMapperName());
    }
}
